...
public enum ProtectCadenceCLIError: Error, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .missingSubcommand:
            return "expected a subcommand such as 'ingest', 'query', 'model', 'annotations', 'auth', 'setup', or 'validate'"
...
public enum ProtectCadenceCLIOutput: Encodable, Sendable {
    ...
    case annotations(AnnotationCommandOutput)
...
public enum ProtectCadenceCLIRunner {
    public static func run(
        ...
        case .annotations:
            return .annotations(try ProtectCadenceAnnotationsRunner.run(arguments: remainingArguments, now: now))
        case .auth:
            return .auth(
